...
def entropy(rows):
	...
	for lbl in c:
		if c[lbl]==0:
			...
			prob = c[lbl]/sum(c.values())
			impurity += -prob*math.log(prob,2)
	return(impurity) 
...
class Leaf:
    ...
    def __init__(self, rows, id, depth):
        self.predictions = max(class_counts(rows))
        self.id = id
        self.depth = depth
...
def print_tree(node, spacing=""):
    ...
    # Base case: we've reached a leaf
    if isinstance(node, Leaf):
        print (spacing + "Predict", node.predictions, "Leafnode id", node.id, "Leafnode Depth", node.depth)
        return

    # Print the question at this node
    print (spacing + str(node.question), "Node id",node.id, "Node Depth", node.depth)
...
def computeAccuracy(rows, node):
	...
	num_accurate = 0
	for row in rows:
		pred_label = classify(row,node)
		if(pred_label) == row[-1]:
			num_accurate += 1
	return round(num_accurate/total_rows,2)
